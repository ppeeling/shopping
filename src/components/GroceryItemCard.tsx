...
import { Check, Edit2, Trash2, StickyNote, AlertCircle, Store, Eye } from 'lucide-react';
...
export const GroceryItemCard: React.FC<GroceryItemCardProps> = ({
  ...
  return (
    <>
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-start gap-3 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              {/* Image Preview Thumbnail if item has image */}
              {item.imageUrl && (
                <div className="mt-2.5 mb-1.5 flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => setIsImageOpen(true)}
                    className="relative group/img overflow-hidden rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 hover:opacity-90 transition-all active:scale-95 text-left"
                    title="Tap to view full image"
                  >
                    <img
                      src={item.imageUrl}
                      alt={item.name}
                      className="h-16 sm:h-20 w-auto max-w-[180px] object-cover rounded-xl"
                    />
                    <div className="absolute inset-0 bg-slate-950/30 opacity-0 group-hover/img:opacity-100 transition-opacity flex items-center justify-center text-white text-xs font-medium gap-1">
      ...
      {item.imageUrl && (
        <ImageModal
          imageUrl={isImageOpen ? item.imageUrl : null}
          title={item.name}
